...
def test_failed_validation_keeps_task_active_and_retry_is_measured(
    tmp_path: Path,
) -> None:
    root = _project(
        tmp_path,
        _python_exit_command(1),
    )
    ...
    with pytest.raises(
        RuntimeError,
        match="Quality validation failed",
    ):
        finish_task(root)
...
def test_finish_reuses_completed_validation_after_partial_failure(
    tmp_path: Path,
) -> None:
    root = _project(
        tmp_path,
        _python_exit_command(0),
    )
    ...
    with session_factory() as session:
        attempt_count = session.scalar(
            ...
            .where(
                TaskValidationAttempt.task_id == task.id
            )
        )
...
def test_run_captures_execution_environment(
    tmp_path: Path,
) -> None:
    root = _project(
        tmp_path,
    ...
    with session_factory() as session:
        environment = session.scalar(
            select(ExecutionEnvironment).where(
                ExecutionEnvironment.run_id == result.run_id
            )
        )

    assert environment is not None

    # The telemetry version must always match the installed AEO package.
...
def test_v02_validation_link_is_backfilled(
    tmp_path: Path,
) -> None:
    root = _project(
        tmp_path,
    ...
    with migrated_factory() as session:
        finalization = session.scalar(
            select(TaskFinalization).where(
                TaskFinalization.task_id == task_id
            )
        )

        attempt = session.scalar(
            select(TaskValidationAttempt).where(
                TaskValidationAttempt.run_id == run_id
            )
        )
...
def test_schema_version_is_persisted(
    tmp_path: Path,
) -> None:
    root = _project(
        tmp_path,
